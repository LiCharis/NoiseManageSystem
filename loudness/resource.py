...
from data.models import Loudness
from total.models import Total


class LoudnessResource(resources.ModelResource):
    ...
    def export(self, queryset=None, *args, **kwargs):
        ...
        for obj in queryset:
            ...
            res[headers.index('图片地址')] = Total.objects.get(id=obj.total.id).loudness_image.path
            data.append(res)
    ...
    left = fields.Field(
        ...
        attribute='total',
        widget=ForeignKeyWidget(Total, 'loudness_left'))

    right = fields.Field(
        column_name='响度右耳-sone',
        attribute='total',
        widget=ForeignKeyWidget(Total, 'loudness_right'))
