...
from .resource import ClarityResource
...
class ClarityManger(ExportActionModelAdmin, admin.ModelAdmin):
    ...
    resource_class = ClarityResource
    ...
    @admin.display(description='语音清晰度左耳-%', ordering='id')
    def left(self, obj):
        obj = Total.objects.get(id=obj.total_id)
        return "%s" % (obj.clarity_left)

    @admin.display(description='语音清晰度右耳-%', ordering='id')
    def right(self, obj):
        obj = Total.objects.get(id=obj.total_id)
        return "%s" % (obj.clarity_right)
    ...
    def showFig(self, obj):
        if not obj.total.clarity_image == " ":
            page_url = "/clarity/get_image/%d" % (obj.id)
            image_url = (MEDIA_URL + obj.total.clarity_image.name)
        else:
            page_url = ""
            image_url = ""
        return format_html(
    ...
    def analyse(self, request, queryset):
        ...
        return HttpResponseRedirect("/clarity/analyse/{}".format('.'.join(str(pk) for pk in selected)))
        # return True
    ...
    def compare(self, request, queryset):
        ...
        selected = queryset.values_list('pk', flat=True)
        return HttpResponseRedirect("/clarity/compare/{}".format('.'.join(str(pk) for pk in selected)))
