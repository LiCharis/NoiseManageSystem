...
class ClarityResource(resources.ModelResource):
    ...
    def export(self, queryset=None, *args, **kwargs):
        ...
        for obj in queryset:
            ...
            res[headers.index('图片地址')] = Total.objects.get(id=obj.total.id).clarity_image.path
            data.append(res)
    ...
    left = fields.Field(
        column_name='语音清晰度左耳-%',
        attribute='total',
        widget=ForeignKeyWidget(Total, 'clarity_left'))

    right = fields.Field(
        column_name='语音清晰度右耳-%',
        attribute='total',
        widget=ForeignKeyWidget(Total, 'clarity_right'))
