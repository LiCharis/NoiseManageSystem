...
class VolatilityManger(ExportActionModelAdmin, admin.ModelAdmin):
    ...
    @admin.display(description='波动度左耳-vacil%', ordering='id')
    def left(self, obj):
        obj = Total.objects.get(id=obj.total_id)
        return "%s" % (obj.volatility_left)

    @admin.display(description='波动度右耳-vacil', ordering='id')
    def right(self, obj):
        obj = Total.objects.get(id=obj.total_id)
        return "%s" % (obj.volatility_right)
    ...
    def showFig(self, obj):
        if not obj.total.volatility_image == " ":
            page_url = "/volatility/get_image/%d" % (obj.id)
            image_url = (MEDIA_URL + obj.total.volatility_image.name)
        else:
            page_url = ""
            image_url = ""
        return format_html(
